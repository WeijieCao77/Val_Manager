...
export function makeChampionsApi(sql, { readBody, json, token, tokenFrom, normalizeId, displayName, fetcher }) {
 const schedule = makeSchedule(sql, fetcher)
 const read = async req => JSON.parse(await readBody(req, 30_000))
 const config = async () => { const [r] = await sql`select value from site_config where key='champions_config'`; return { enabled:true, popup:true, overrides:{}, ...r?.value } }
 return { async route(req,res,path,url) {
  ...
  if(admin) {
   ...
   if(body.action==='config') {
    ...
    if(body.day) {
     if(!/^2026-\d{2}-\d{2}$/.test(body.day)||body.day<CHAMPIONS.start||body.day>CHAMPIONS.end) { json(res,400,{ok:false,why:'日期不在赛事范围内。'}); return true }
