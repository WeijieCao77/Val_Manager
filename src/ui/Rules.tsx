...
const buildSections = (drawn: boolean): Section[] => [
  ...
  },
  {
    ...
    lede: '每两名选手之间一个数，−100 到 +100，双向相同，说的是这两个人处得怎么样。'
      ...
      + '位置互补（决斗者↔先锋、控场↔哨卫）+3、团队配合和沟通越高越好相处。',
    up: [
  ...
  },
  {
    ...
    up: [
      { t: '节奏 / 侵略性', d: '每点节奏攻 +0.035 守 −0.022，每点侵略性攻 +0.028 守 −0.015（50 为零点）。'
        ...
        + '<b>双哨卫进攻端乘 0.5、防守端乘 2.2</b>，往左拉才厚。标准阵容不乘。' },
  ...
  },
  {
    ...
    lede: '地图熟练度是多熟这张图；阵容熟练度是多熟<b>这张图上这五个英雄</b>。每张图一个数，'
      + '0~100，<b>50 是中立</b>。别的俱乐部普通难度在 50，困难 62，职业 70。',
    up: [
      { t: '正式比赛打一张图', d: '+8。' },
      { t: '训练赛打一张图', d: '+6。' },
    ...
    use: [
      ...
      '但一套不动也最容易被研究透，见「对手针对」。',
    ],
  },
  {
    key: 'scout',
    title: '对手针对与难度',
    lede: '赢得越多，别的队越研究你的录像。<b>针对度</b> 0~100，<b>被摸透</b>按每张图算。'
      + 'AI 打你时多出的备战分 = 上限 × 针对度 × (35% + 65% × 被摸透)。上限：普通 6，困难 8，职业 10。',
    up: [
      { t: '赢一场正式比赛', d: '针对度 +3。' },
      { t: '拿冠军', d: '赛区冠军 +8，国际赛冠军 +15。' },
      { t: '同一张图连打同一套', d: '五个英雄和四条滑杆都没变，打三场就被摸透。' },
    ],
    down: [
      { t: '输一场', d: '针对度 −8。赢球率低于七成三，它会往下走。' },
      { t: '换英雄', d: '换一个英雄，对手的功课作废五分之一。' },
      { t: '拨滑杆', d: '任意一条动 10 以上，滑杆那部分（35%）作废。' },
      { t: '过冬', d: '针对度剩六成，被摸透减半。训练赛不算。' },
    ],
    use: [
      '<b>宿敌</b>：赛季末针对度 ≥50 时，本季被你赢得最多的一级联赛队会盯上你：下赛季对你的备战 ×1.4，多 150 万预算，转会窗口每周买人。',
      '<b>困难 / 职业</b>：个人能力（加权后）超过 88 / 86 的部分只算一半 / 四成；AI 练到 99，阵容熟练度更高，冠军引起的反弹更大。普通难度超过 90 的部分算六成。',
      '难度开档时选，之后在「存档」页只能往上调。',
      '找你加薪的人，没涨到身价八成以上，之后每周不满 +2（困难 +3，职业 +4）、士气 −1。',
    ],
  },
  {
    key: 'trust',
