...
/** Read while the tab is in view: the page refreshes every minute, the reminder only on arrival and on coming back. */
function useFeed(poll:boolean) {
 const [feed,setFeed]=useState<Feed|null>(null), [error,setError]=useState('')
 useEffect(()=>{let alive=true;const load=()=>{if(document.visibilityState==='hidden')return;void request('').then(d=>{if(alive){setFeed(d);setError('')}}).catch(e=>{if(alive)setError(e.message)})}
  const onVisible=()=>{if(document.visibilityState==='visible')load()}
  load();const timer=poll?setInterval(load,60_000):undefined;document.addEventListener('visibilitychange',onVisible)
  return()=>{alive=false;if(timer)clearInterval(timer);document.removeEventListener('visibilitychange',onVisible)}},[poll])
 return {feed,error}
}
function Schedule({feed,day}:{feed:Feed;day:string}) {
 ...
 {rows.length?rows.map(m=><div className="champ-match" key={m.id}><time dateTime={m.start}>{m.time}</time><b>{m.a==='TBD'?'对阵待定':m.a}</b><span>VS</span><b>{m.b==='TBD'?'对阵待定':m.b}</b></div>):<p className="champ-no-match">{day===feed.today?'今日':'当天'}暂无已公布比赛。休赛日也可以为选手留言。</p>}
...
export function ChampionsNotice({enabled,inCards=false}:{enabled:boolean;inCards?:boolean}) {
 const {feed}=useFeed(false),[open,setOpen]=useState(false),shown=useRef(false)
 const ref=useDialogFocus(()=>setOpen(false),open)
 useEffect(()=>{
  if(shown.current||!enabled||!feed||!feed.popup||feed.today<feed.event.start||feed.today>feed.event.end||!feed.matches.some(m=>m.day===feed.today))return
  ...
  // once per page as well: without storage the key never sticks, and the feed re-reads on every return to the tab
  shown.current=true
  setOpen(true)
 ...
 <button className="champ-close" aria-label="关闭今日赛事提醒" onClick={()=>setOpen(false)}>✕</button><div className="champ-city" aria-hidden="true">上海</div><p className="champ-kicker">2026 无畏契约全球冠军赛</p><h2 id="champ-notice-title">{feed.today===feed.event.start?'上海，今日开战。':'今天，为他们呐喊。'}</h2><p>把想说的话，留给你支持的选手。</p><Schedule feed={feed} day={feed.today}/><a className="champ-cta" href="/champions" onClick={e=>{if(!inCards)return;e.preventDefault();setOpen(false);location.hash='#champions'}}>查看赛程 · 留下应援</a><button className="champ-later" onClick={()=>setOpen(false)}>先去玩，稍后再看</button></section></div>
}
/** `embedded` inside 开瓦包: its own nav already frames the page, and a hash link would change the tab. */
export default function ChampionsPage({embedded=false}:{embedded?:boolean}) {
 const {feed,error}=useFeed(true),[day,setDay]=useState(''),[rows,setRows]=useState<Message[]>([]),[mine,setMine]=useState<Message[]>([]),[more,setMore]=useState(false),[loading,setLoading]=useState(false),[wallError,setWallError]=useState('')
 ...
 return <div className="champ-page">{!embedded&&<header className="champ-page-nav"><a href="/">← 猪之家首页</a><a href="/cards">返回开瓦包</a></header>}
 <section className="champ-hero"><div className="champ-city" aria-hidden="true">上海</div><p className="champ-kicker">2026 无畏契约上海全球冠军赛 · Beta</p><h1>把主场的声音，<br/>送到他们身边。</h1><p>为 CN 呐喊，也为每一位你相信的选手加油。</p><div className="champ-hero-links"><button type="button" className="champ-cta" onClick={()=>document.getElementById('champ-compose')?.scrollIntoView({behavior:'smooth',block:'start'})}>写下你的应援</button><span>09.24 — 10.18 · 上海</span></div></section>
