...
for (const tens of [3, 10, 30]) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    while (g.cup && !g.cup.done) {
      ...
      const r = playArenaMatch(g.squad, level, opp, 3, i * 71 + round)
  ...
  cupPer.push({ rating, perLeg: net / legs, perRun: net / N, legs: legs / N })
  console.log(`  阵容分 ${rating}  平均 ${(legs / N).toFixed(1)} 轮/次  `
    + `净收益 ${money(net / N)}/次 = ${money(net / legs)}/轮  每点体力 ${money(net / legs / STAMINA_COST.cup)}`
    + `  （报名费 ${CUP_ENTRY} 已扣）`)
}
...
console.log(`  杯赛（阵容分 ${best.rating}）：全打杯赛`
  + ` → ${money(perDayStamina * (best.perLeg / STAMINA_COST.cup))} 金币/天`)
