...
import { agentFit, agentMod, agentRoleGaps, autoAgents, OFF_ROLE } from '../src/engine/agents'
import { AGENT_ROLE, MAPS, MAP_META, mapCn } from '../src/engine/content'
...
// ---- a pick out of position costs, and training buys it back
{
  ...
  check('someone else\'s job costs the full penalty',
    Math.abs((1 - off) - (OFF_ROLE + (duelist.agentPool.includes(wrongAgent) ? 0 : 0.03))) < 1e-9,
    `×${off.toFixed(3)}`)
  ...
  check('and halfway there costs half of it',
    Math.abs(half - (OFF_ROLE * 0.5 + 0.03)) < 1e-9, `−${(half * 100).toFixed(1)}%`)
}
